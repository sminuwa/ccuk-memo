            ...
            <div class="widget-content widget-content-area br-6">
                <div class="float-right mb-lg-4">
                    <button class="btn btn-primary add">
                        <i class="las la-plus-circle"></i> Add Allowance
                    </button>
                </div>
                <h4 class="table-header mb-4">Manage Allowances</h4>
...
@section('modals')
    <div id="addEditAllowanceModal" class="modal animated {{ modalAnimation() }}" role="dialog">
        <div class="modal-dialog {{ modalClasses() }}">
            <!-- Modal content-->
            <div class="modal-content {{ modalPadding() }}">
                <div class="modal-header">
                    <h5 class="modal-title">Add/Edit Allowances</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="addEditAllowanceForm" method="post" action="{{ route('payroll.allowances.store') }}">
                        @csrf
                        <input type="hidden" value="" name="payroll_allowance_id" id="payroll_allowance_id"/>
                        <div class="row">
                            <div class="col-md-12">
    ...
    <form action="" method="post" id="deleteAllowanceForm">
        @csrf
        {{ method_field('DELETE') }}
    </form>
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            $('body').on('click', '.edit, .add', function () {
                let payroll_allowance_id = $(this).attr('id')
                ...
                if (payroll_allowance_id) {
                    $('#payroll_allowance_id').val(payroll_allowance_id)
                    $('#item_id').val(item_id)
                    $('#amount').val(amount)
                ...
                $('#addEditAllowanceModal').modal();
                $('.btnSave').click(function () {
                    $('#addEditAllowanceForm').submit();
                })
            });
            ...
            $(document).on('click','.delete', function () {
                ...
                let url = "{{ route('payroll.allowances.destroy',':id') }}"
                url = url.replace(':id', payroll_deduction_id);
                $('#deleteAllowanceForm').attr('action', url)
                swal({
                    title: 'Are you sure?',
